...
const MarketTurnos: React.FC = () => {
  ...
  useEffect(() => {
    // Escuchar solo las ofertas disponibles o las que ya están asignadas a este empleado
    const q = query(
      collection(db, 'solicitudes_turnos'),
      where('estado', 'in', ['disponible', 'asignado', 'completado'])
    );
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        if (turno.estado === 'disponible') {
          // Verificar si el guardia está en la lista de permitidos (o si la lista está vacía/no existe para retrocompatibilidad)
          if (!turno.guardias_permitidos || turno.guardias_permitidos.length === 0 || turno.guardias_permitidos.includes(currentUser?.uid || '')) {
            data.push(turno);
          }
        } else {
          // Siempre mostrar los que ya están asignados o completados para dar feedback
          data.push(turno);
        }
      });
      // Sort: My assigned first, then newest
      data.sort((a, b) => {
        if (a.id_colaborador_asignado === currentUser?.uid) return -1;
        if (b.id_colaborador_asignado === currentUser?.uid) return 1;
  ...
  const handleAceptarTurno = async (solicitud: SolicitudTurno) => {
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        // El turno está disponible, lo asignamos atómicamente
                ...
                >
                  <div className="flex flex-col md:flex-row justify-between md:items-center gap-6">
                    ...
                    <div className={`flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4 border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-6 ${isPastAndNotMine ? 'border-red-500/10' : 'border-slate-100'}`}>
                      ...
                      {isMine ? (
                         <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl font-black text-sm border border-emerald-100">
                           <CheckCircle size={18} />
                           ¡TU TURNO!
                         </div>
                      ) : (
