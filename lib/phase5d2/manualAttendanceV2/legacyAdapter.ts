import { ManualAttendanceV2, ManualAttendanceLegacy } from './types';

export function isManualAttendanceV2(data: any): data is ManualAttendanceV2 {
  return (
...
export function isManualAttendanceLegacy(data: any): data is ManualAttendanceLegacy {
...
export function adaptLegacyManualAttendance(data: any, documentId: string): Partial<ManualAttendanceV2> {
  if (isManualAttendanceV2(data)) {
    return data;
  }
  ...
  return {
    ...
    isLegacy: true as any, // TypeScript partial hack to distinguish, but the type says false.
