...
const qaUid = args.find(a => a.startsWith('--uid='))?.split('=')[1];
const sucursalId = args.find(a => a.startsWith('--sucursal='))?.split('=')[1];
const mesOperativo = args.find(a => a.startsWith('--mes='))?.split('=')[1];
...
if (!qaUid || !sucursalId || !mesOperativo || !requestId || !operator || !projectArg) {
  console.error('[ERROR] Falta uno de los parámetros obligatorios.');
  console.error('Uso: node activate_attendance_v2_qa.js --project=<PROJECT_ID> --uid=<UID> --sucursal=<SUC_ID> --mes=<YYYY-MM> --request-id=<REQ_ID> --operator=<OPERATOR> [--dry-run]');
  process.exit(1);
}
...
async function execute() {
  console.log('================================================');
  console.log(' ACTIVACIÓN QA PARA DUAL WRITE Y SHADOW READING ');
  console.log('================================================');
  ...
  console.log(`- Usuario (UID): ${qaUid}`);
  console.log(`- Sucursal:      ${sucursalId}`);
  console.log(`- Mes Operativo: ${mesOperativo}`);
  console.log(`- Ticket/Reg:    ${requestId}`);
  console.log(`- Operador:      ${operator}`);
  
  const confirmationString = `CONFIRMAR ${projectArg} ${qaUid} ${sucursalId} ${mesOperativo}`;
  ...
  rl.question(`\nPara proceder, escribe exactamente:\n${confirmationString}\n> `, async (answer) => {
    ...
    try {
      await db.runTransaction(async (transaction) => {
        const auditRef = db.collection('AuditoriaAcciones').doc(`qa-activation-${Date.now()}`);
        
        transaction.set(auditRef, {
          action: 'ACTIVATE_QA_V2',
          requestId,
          operator,
          projectId: projectArg,
          qaUid,
          sucursalId,
          operationalMonth: mesOperativo,
          previousState: stateBefore,
          newState: {
            enabled: true,
            allowedUsers: [qaUid],
            allowedSites: [sucursalId],
            mesQA: mesOperativo
          },
        ...
        for (const flag of flags) {
          ...
          transaction.update(ref, {
            ...
            allowedSites: [sucursalId],
            mesQA: mesOperativo,
            lastActivatedByReq: requestId,
            updatedAt: FieldValue.serverTimestamp()
          });
        }
      });

      console.log('\n[SUCCESS] Feature Flags activados y auditoría registrada.');
      ...
      console.error('[FATAL] Error en la transacción de activación:', error);
      process.exit(1);
    }
    process.exit(0);
  });
